      ...
      switch (this.os) {
        case 'MacIntel' || 'Mac68K' || 'MacPPC' || 'iPhone' || 'iPad':
          ...
          fetch('https://api.github.com/repos/figma-plus/installer/releases', {
            ...
            downloadLink.href = releases[0].assets.find(function (asset) {
              return asset.name.toLowerCase().includes('mac');
        ...
        case 'Win32' || 'Win16' || 'Windows':
          ...
          fetch('https://api.github.com/repos/figma-plus/installer/releases', {
            ...
            downloadLink.href = releases[0].assets.find(function (asset) {
              return asset.name.toLowerCase().includes('exe');
            }).browser_download_url;
          });
          break;
    ...
    value: function handleModal() {
      ...
      Object.keys(modalsLinks).forEach(function (el) {
        ...
        modalOpen.addEventListener('click', function () {
          ...
          if (el === 'video') _this.player.play();
        });
        modalClose.addEventListener('click', function () {
          doodle.style.display = 'block';
          modalTarget.classList.remove('is-open');
          if (el === 'video') _this.player.stop();
        });
      });
    }
  }, {
    key: "handelVideo",
    value: function handelVideo() {
      this.player = new Plyr('#player');
      this.player.on('ended', function (event) {
        var doodle = document.querySelector('css-doodle');
        var modalTarget = document.querySelector('.video-teaser');
        doodle.style.display = 'block';
        modalTarget.classList.remove('is-open');
      });
    }
